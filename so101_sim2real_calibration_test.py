...
class SO101CalibrationTester:
    ...
    def read_real_robot_angles(self):
        ...
        try:
            qpos = self.real_agent.get_qpos()  # This returns a numpy array
            
            # Handle numpy array (not tensor)
            if isinstance(qpos, np.ndarray):
                qpos_rad = qpos.flatten()  # Flatten in case it's 2D
            else:
                # If it's a tensor (shouldn't happen based on the code, but just in case)
                qpos_rad = qpos.cpu().numpy().flatten()
            ...
            print(f"  qpos type: {type(qpos) if 'qpos' in locals() else 'not available'}")
            print(f"  qpos shape: {qpos.shape if 'qpos' in locals() and hasattr(qpos, 'shape') else 'not available'}")
            return None
